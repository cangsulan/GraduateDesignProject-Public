# -*- coding: utf-8 -*-
"""
神经网络融合方案实验 (V5.5) - 修正版，加入随机种子控制

数据使用原则：
1. 测试集（dataset_test.csv）完全不可见
2. 训练数据（15000条）充分利用
3. 每次神经网络训练前重置随机种子，确保结果可复现
...
def train_fusion_net(train_features, train_labels, 
                     ...
                     logger, max_epochs=200, patience=20, lr=0.001, seed=SEED):
    set_seed(seed)
...
def run_experiments():
    log_file = os.path.join(LOG_DIR, f'nn_fusion_v55_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.txt')
    logger = ExperimentLogger(log_file)
    
    logger.log_section("神经网络融合实验 (V5.5) - 修正版，加入随机种子控制")
    logger.log("数据使用原则：")
    logger.log("  1. 测试集完全不可见")
    logger.log("  2. 训练数据(15000条)充分利用")
    logger.log("  3. 训练集80% + 验证集20%")
    logger.log("  4. 每次训练前重置随机种子，确保结果可复现")
    ...
    logger.log_section("阶段7: 神经网络融合实验（多次运行验证）")
    ...
    for epochs in epoch_configs:
        ...
        for run in range(3):
            ...
            logger.log(f"    运行{run+1} F1={metrics['f1']:.4f}")
        
        avg_f1 = np.mean(all_f1_scores)
        std_f1 = np.std(all_f1_scores)
        
        logger.log(f"  平均F1={avg_f1:.4f} (std={std_f1:.4f})")
        
        best_run_idx = np.argmax(all_f1_scores)
        best_metrics = all_metrics[best_run_idx]
    ...
    logger.log("\n一、检测性能对比:")
    logger.log("-" * 100)
    logger.log(f"{'方案':<25} {'F1(最佳)':<12} {'F1(平均)':<12} {'F1(标准差)':<12} {'Precision':<14} {'Recall':<12}")
    logger.log("-" * 100)
    for r in sorted(results, key=lambda x: x['avg_f1'], reverse=True):
        m = r['metrics']
        logger.log(f"{r['name']:<25} {m['f1']:<12.4f} {r['avg_f1']:<12.4f} {r['std_f1']:<12.4f} {m['precision']:<14.4f} {m['recall']:<12.4f}")
    logger.log("-" * 100)
    ...
    logger.log(f"\n最佳方案(按平均F1): {best['name']}, 平均F1={best['avg_f1']:.4f}")
    
    results_json = os.path.join(LOG_DIR, 'nn_fusion_v55_results.json')
